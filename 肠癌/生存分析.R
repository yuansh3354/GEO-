...
pd = na.omit(pd)
pd$label = ifelse(pd$stage > 2 ,'hig','low') 
geoid = rownames(pd)
sit = which(colnames(df_expr == geoid) %in% geoid)
df_expr =df_expr[,sit]
save(pd,df_expr, file = 'data.Rdata')
if(T){
  group_list = as.character(pd[,4])
...
if(T){
  ...
  top = top[1:100,]
  int_gene = rownames(top[which(top$adj.P.Val<0.05),])
...
if(T){
  ...
  n= 1# 1 2
  survival_table$TXN2 = ifelse(t(survival_exp[n,]) > mean(as.numeric(survival_exp[n,])),'low','hig')
  meta = na.omit(survival_table)
  meta$time = as.numeric(meta$time)
  rownames(survival_exp[n,])
  sfit1 <- survfit(Surv(time, status)~TXN2, data=meta) #primary_ER/CTC_ER
  ...
  ggsurvplot(sfit1,palette =c("#1EB2A6","#F67575"),
             ...
             ncensor.plot = TRUE)
  ggsave(paste(rownames(survival_exp[n,]),'.pdf',sep = ''))
